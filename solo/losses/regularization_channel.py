...
class blance_loss(nn.Module):
    ...
    def forward(self, mask_norm_c, norm_c_t, batch_size, 
                den_target, gamma, p):
        norm_c = mask_norm_c
...
class Loss(nn.Module):
    ...
    def forward(self, flops_real, flops_mask, flops_ori, batch_size, 
                den_target, lbda, mask_norm_c, norm_c_t,
                gamma, p):
        # closs = self.task_loss(output, targets)
        sloss = self.spar_loss(flops_real, flops_mask, flops_ori, batch_size, den_target, lbda)
        bloss = self.balance_loss(mask_norm_c, norm_c_t, batch_size,
                                  den_target, gamma, p)
        return sloss, bloss
